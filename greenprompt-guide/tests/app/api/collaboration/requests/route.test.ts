...
describe("API :: Collaboration :: Requests", () => {
    ...
    describe("GET", () => {
        ...
        it("should return 403 if user is not an admin", async () => {
            (getSession as Mock).mockResolvedValue("testuser");
            (getUserByUsername as Mock).mockResolvedValue({ role: 'USER' });
            const response = await GET();
        ...
        it("should return 200 with requests for admin", async () => {
            ...
            (getUserByUsername as Mock).mockResolvedValue({ role: 'ADMIN' });
            (listAllCollaborationRequests as Mock).mockResolvedValue([{ id: 1 }]);
            const response = await GET();
        ...
        it("should return 500 on error", async () => {
            (getSession as Mock).mockResolvedValue("admin");
            (getUserByUsername as Mock).mockResolvedValue({ role: 'ADMIN' });
            (listAllCollaborationRequests as Mock).mockRejectedValue(new Error("DB error"));
            const response = await GET();
    ...
    describe("POST", () => {
        it("should create a request and save the file", async () => {
            ...
            (createCollaborationRequest as Mock).mockResolvedValue({ id: 1, practiceTitle: "title" });
            (getCollaborationPdfStorageDir as Mock).mockReturnValue("/tmp/storage");
            (getCollaborationPdfStoragePath as Mock).mockImplementation(p => `/tmp/storage/${p}`);
            (randomUUID as Mock).mockReturnValue("mock-uuid");
            (createCollaborationRequestDir as Mock).mockResolvedValue(undefined);
            (writeCollaborationRequestFile as Mock).mockResolvedValue(undefined);
        ...
        it("should return 400 if required field is missing", async () => {
            (getSession as Mock).mockResolvedValue("testuser");
            (getUserByUsername as Mock).mockResolvedValue({ username: "testuser" });
            const formData = new FormData();
        ...
        it("should delete created request if file write fails", async () => {
            (getSession as Mock).mockResolvedValue("testuser");
            (getUserByUsername as Mock).mockResolvedValue({ username: "testuser" });
            (createCollaborationRequest as Mock).mockResolvedValue({ id: 123 });
            (writeCollaborationRequestFile as Mock).mockRejectedValue(new Error("Storage error"));
