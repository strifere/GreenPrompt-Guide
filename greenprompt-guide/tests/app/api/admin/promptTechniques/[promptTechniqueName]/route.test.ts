...
describe("API :: Admin :: PromptTechniques :: [promptTechniqueName]", () => {
  ...
  describe("PATCH", () => {
    ...
    it("should return 404 if prompt technique is not found", async () => {
        ...
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue(null);
        request = new NextRequest("http://localhost", { method: 'PATCH', body: JSON.stringify(techniquePayload) });
    ...
    it("should return 400 if description is missing", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue({ name: 'test-technique' });
    ...
    it("should return 400 if references are not found", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue({ name: 'test-technique' });
        (prisma.reference.findMany as Mock).mockResolvedValue([{ title: 'ref1' }]);
        request = new NextRequest("http://localhost", { method: 'PATCH', body: JSON.stringify(techniquePayload) });
    ...
    it("should update the prompt technique successfully", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue({ name: 'test-technique' });
        (prisma.reference.findMany as Mock).mockResolvedValue([{ title: 'ref1' }, { title: 'ref2' }]);
        (prisma.promptTechnique.update as Mock).mockResolvedValue({ name: 'test-technique', ...techniquePayload});
    ...
    it("should return 500 on server error", async () => {
        ...
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue({ name: 'test-technique' });
        (prisma.reference.findMany as Mock).mockResolvedValue([{ title: 'ref1' }, { title: 'ref2' }]);
        (prisma.promptTechnique.update as Mock).mockRejectedValue(new Error("DB error"));
  ...
  describe("DELETE", () => {
    ...
    it("should delete the prompt technique successfully", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue({ name: 'test-technique' });
        (prisma.promptTechnique.delete as Mock).mockResolvedValue({});
        const response = await DELETE(request, context);
        const json = await response.json();
        expect(response.status).toBe(200);
    ...
    it("should return 500 on server error", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue({ name: 'test-technique' });
        (prisma.promptTechnique.delete as Mock).mockRejectedValue(new Error("DB error"));
        const response = await DELETE(request, context);
        const json = await response.json();
        expect(response.status).toBe(500);
