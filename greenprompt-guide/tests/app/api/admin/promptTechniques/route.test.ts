...
describe("API :: Admin :: PromptTechniques", () => {
  ...
  describe("POST", () => {
    ...
    it("should return 409 if prompt technique name already exists", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue({ name: 'New Technique' });
        request = new NextRequest("http://localhost", { method: 'POST', body: JSON.stringify(techniquePayload) });
    ...
    it("should create the prompt technique successfully", async () => {
        ...
        (prisma.promptTechnique.create as Mock).mockResolvedValue({ ...techniquePayload});
    ...
    it("should return 500 on server error", async () => {
        ...
        (prisma.promptTechnique.findUnique as Mock).mockResolvedValue(null);
        (prisma.reference.findMany as Mock).mockResolvedValue([{ title: 'ref1' }, { title: 'ref2' }]);
        (prisma.promptTechnique.create as Mock).mockRejectedValue(new Error("DB error"));
