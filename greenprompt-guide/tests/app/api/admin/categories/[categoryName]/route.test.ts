...
describe("API :: Admin :: Categories :: [categoryName]", () => {
  ...
  describe("PATCH", () => {
    ...
    it("should return 400 for invalid tactic", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.category.findUnique as Mock).mockResolvedValue({ name: 'test-category' });
    ...
    it("should update the category successfully", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.category.findUnique as Mock).mockResolvedValue({ name: 'test-category' });
        (prisma.category.update as Mock).mockResolvedValue({ name: 'test-category', description: 'new description', tactic: 'GREEN_PRACTICE' });
    ...
    it("should return 500 on server error", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.category.findUnique as Mock).mockResolvedValue({ name: 'test-category' });
        (prisma.category.update as Mock).mockRejectedValue(new Error("DB error"));
  ...
  describe("DELETE", () => {
    ...
    it("should delete the category successfully", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.category.findUnique as Mock).mockResolvedValue({ name: 'test-category' });
        (prisma.category.delete as Mock).mockResolvedValue({});
    ...
    it("should return 500 on server error", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.category.findUnique as Mock).mockResolvedValue({ name: 'test-category' });
        (prisma.category.delete as Mock).mockRejectedValue(new Error("DB error"));
