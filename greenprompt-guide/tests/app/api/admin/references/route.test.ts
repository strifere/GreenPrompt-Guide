...
describe("API :: Admin :: References", () => {
  ...
  describe("POST", () => {
    ...
    it("should return 409 if reference title already exists", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.reference.findUnique as Mock).mockResolvedValue({ title: 'New Reference' });
        request = new NextRequest("http://localhost", { method: 'POST', body: JSON.stringify(referencePayload) });
    ...
    it("should create the reference successfully", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.reference.findUnique as Mock).mockResolvedValue(null);
        (prisma.reference.create as Mock).mockResolvedValue({ ...referencePayload });
    ...
    it("should return 500 on server error", async () => {
        (requireAdmin as Mock).mockResolvedValue({ ok: true });
        (prisma.reference.findUnique as Mock).mockResolvedValue(null);
        (prisma.reference.create as Mock).mockRejectedValue(new Error("DB error"));
