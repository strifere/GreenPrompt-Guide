...
const mockCRFindUnique = prisma.collaborationRequest.findUnique as Mock;
const mockJobUpsert = prisma.analysisJob.upsert as Mock;
const mockJobFindUnique = prisma.analysisJob.findUnique as Mock;
...
describe("/api/admin/requests/[requestId]/analyze", () => {
    ...
    describe("POST", () => {
        it("should return 401 if not authenticated", async () => {
            ...
            const response = await POST({} as Request, { params:  Promise.resolve({ requestId: "1" }) });
            expect(response.status).toBe(401);
        });

        it("should return 400 for an invalid request ID", async () => {
            mockGetSession.mockResolvedValue("admin");
            const response = await POST({} as Request, { params:  Promise.resolve({ requestId: "invalid" }) });
        ...
        it("should return 404 if request not found", async () => {
            ...
            const response = await POST({} as Request, { params:  Promise.resolve({ requestId: "999" }) });
    ...
    describe("GET", () => {
        it("should return 401 if not authenticated", async () => {
            mockGetSession.mockResolvedValue(null);
            const response = await GET({} as Request, { params:  Promise.resolve({ requestId: "1" }) });
            expect(response.status).toBe(401);
        });

        it("should return 400 for an invalid request ID", async () => {
            mockGetSession.mockResolvedValue("admin");
            const response = await GET({} as Request, { params:  Promise.resolve({ requestId: "invalid" }) });
            expect(response.status).toBe(400);
        });
