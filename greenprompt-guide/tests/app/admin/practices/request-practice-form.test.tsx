...
describe("RequestPracticeForm Component", () => {
	...
	it("renders PracticeForm component", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} />);
	...
	it("sets correct submitUrl with request ID", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestId={456} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("pre-fills practice title from request", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestTitle="Green AI Optimization" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("pre-fills practice description from request description", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestDescription="Full detailed description here" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("pre-fills reference title from request title", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestTitle="Paper Title" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("pre-fills reference abstract from request summary", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestSummary="Abstract summary" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("pre-fills reference link from request reference link", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestReferenceLink="https://arxiv.org/paper123" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("pre-fills examples from request examples", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestExamples="Original example\nImproved example" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("handles null request examples", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestExamples={null} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("preserves request title in source when provided", () => {
		...
		render(<RequestPracticeForm references={[]} {...defaultProps} requestTitle={requestTitle} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("preserves request summary in source when provided", () => {
		...
		render(<RequestPracticeForm references={[]} {...defaultProps} requestSummary={requestSummary} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("preserves request description in source when provided", () => {
		...
		render(<RequestPracticeForm references={[]} {...defaultProps} requestDescription={requestDescription} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("handles very long request title", () => {
		...
		render(<RequestPracticeForm references={[]} {...defaultProps} requestTitle={longTitle} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("handles newlines in request examples", () => {
		...
		render(<RequestPracticeForm references={[]} {...defaultProps} requestExamples={examplesWithNewlines} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("handles empty string request examples", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestExamples="" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("handles empty string request summary", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestSummary="" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("handles empty string request reference link", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestReferenceLink="" />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("correctly sets request ID in URLs for multiple requests", () => {
		const { rerender } = render(<RequestPracticeForm references={[]} {...defaultProps} requestId={1} />);
		...
		rerender(<RequestPracticeForm references={[]} {...defaultProps} requestId={999} />);
	...
	it("displays readable text showing redirect path", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} requestId={777} />);
	...
	it("handles request with multiple categories", () => {
		...
		render(<RequestPracticeForm references={[]} {...defaultProps} categories={manyCategories} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("handles empty categories array", () => {
		render(<RequestPracticeForm references={[]} {...defaultProps} categories={[]} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
	...
	it("integrates all request data into source object correctly", () => {
		...
		render(<RequestPracticeForm references={[]} {...customProps} />);
		const form = screen.getByTestId("practice-form");
		const props = JSON.parse(form.getAttribute("data-props") || "{}");
