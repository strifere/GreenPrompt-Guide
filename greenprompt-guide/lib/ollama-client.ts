...
export async function analyzeRequestWithOllama(
  ...
  const queryModel = async (subPrompt: string): Promise<unknown> => {
  ...
  // Narrows an unknown JSON blob to a plain object so property access is safe.
  // Returns an empty object if the value is not an object (e.g. null, array, primitive).
  ...
  const b2 = asRecord(dataBlock2);
  const b3 = asRecord(dataBlock3);
  const b4 = asRecord(dataBlock4);

  const practice  = asRecord(b1.practice);
  const reference = asRecord(b2.reference);

  const finalizedResult: OllamaExtractionResult = {
    practice: {
      name:        (practice.name        as string)  ?? "Unknown Practice",
      description: (practice.description as string)  ?? "",
      greenScore:  (practice.greenScore  as number)  ?? 50,
      tactic:      (practice.tactic      as Practice["tactic"]) ?? "GREEN_PRACTICE",
      categories:  (practice.categories  as string[]) ?? [],
    },
    reference: {
      title:            (reference.title            as string)       ?? "Unknown Title",
      authors:          (reference.authors          as string)       ?? "Unknown Authors",
      year:             (reference.year             as number)       ?? new Date().getFullYear(),
      abstract:         (reference.abstract         as string)       ?? "",
      keywords:         (reference.keywords         as string)       ?? "",
      studyType:        (reference.studyType        as string)       ?? "",
      domain:           (reference.domain           as string|null)  ?? null,
      task:             (reference.task             as string|null)  ?? null,
      venue:            (reference.venue            as string|null)  ?? null,
      toolAvailability: (reference.toolAvailability as string|null)  ?? null,
