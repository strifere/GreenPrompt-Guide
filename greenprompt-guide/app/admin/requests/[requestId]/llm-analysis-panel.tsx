...
export function LlmAnalysisPanel({ requestId, existingJob }: Readonly<LlmAnalysisPanelProps>) {
  ...
  const pollingAttempt = state.kind === "polling" ? state.attempt : null;
    ...
    const poll = async () => {
      try {
        ...
        if (data.status === "DONE") {
          ...
          const next = getNextInterval(pollingAttempt ?? 0);
  ...
  }, [state.kind, pollingAttempt, requestId]);
...
function ExtractionPreview({
  ...
  return (
    <div>
      ...
      {result.promptTechniques.length > 0 && (
        <Section title={`Prompt techniques (${result.promptTechniques.length})`}>
          {result.promptTechniques.map((t) => (
            <div key={t.name} style={{ marginBottom: "12px" }}>
              ...
              {t.example && (
                <p style={{ margin: "4px 0 0", fontStyle: "italic", color: "var(--text-muted)", fontSize: "0.9rem" }}>
                  Example: {t.example}
                </p>
              )}
            </div>
